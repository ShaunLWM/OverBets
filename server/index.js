...
app.use(
	cors({
		origin: ["http://localhost:3002", "http://localhost:5000"], // allow to server to accept request from different origin
...
app.get("/", (req, res) => {
	// get all bets info aka homepage
...
function emitNewBet({ matchId, battletag, coins, img, odds, percentage }) {
...
io.on("connection", socket => {
	...
	socket.on("match:bet:new", ({ coins, token, side, matchId }) => {
		jwt.verify(token, process.env.JWT_SECRET, async (error, decoded) => {
			if (error) return console.log("Failed to decode token");
			const { user_id: uid } = decoded;
			const mid = Number(matchId);
			...
			if (matchData.match.match_status !== "MATCH_OPEN")
				return socket.emit("match:bet:new:end", { success: false, msg: "Match is not open for betting" });

...
function IndividualMatchContainer() {
	...
	useDeepCompareEffect(() => {
		async function fetchMatch() {
			const matches = await fetch(`http://localhost:3003/matches/${matchId}`);
		...
		if (state["matches"].length < 1)
			// TODO: What if totally no match at all?
			fetchMatch();
		else setCurrentMatch(state["matches"].find(match => match.match.match_id === Number(matchId)));
	}, [state["matches"], matchId]);

	const handleBetClick = () => {
		const userCurrentCoins = state["user"]["user_coins"];
		if (typeof userCurrentCoins === "undefined") return;
		const coins = Number(currentBetInput);
	...
	const handleBetInput = e => setCurrentBetInput(e.target.value);
			...
			{typeof currentMatch["match"] !== "undefined" && (
				<>
					<Grid item xs={12} sm={12} md={6}>
						<MatchCard match={currentMatch["match"]} key={currentMatch["match"]["match_id"]} />
					</Grid>
					<Grid item xs={12} sm={12} md={6}>
						<MatchBets users={currentMatch["users"]} />
					</Grid>
				</>
			)}
			...
			{typeof currentMatch["match"] !== "undefined" && typeof state["user"]["user_id"] !== "undefined" ? (
				<>
					<Grid container item xs={12} sm={12} md={6}>
						<Grid item xs={4} sm={4}>
							<BetMatchCoin onChange={handleBetInput} value={currentBetInput} />
						</Grid>
						<Grid item xs={4} sm={4}>
							<BetMatchTeam
								handleTeamChange={handleTeamChange}
								teamOneName={currentMatch["match"]["teamOne"]["team_fullname"]}
								teamTwoName={currentMatch["match"]["teamTwo"]["team_fullname"]}
							/>
						</Grid>
						<Grid item xs={4} sm={4}>
							<Button variant="contained" onClick={handleBetClick}>
								Bet
							</Button>
						</Grid>
					</Grid>
